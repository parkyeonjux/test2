...
if ($checkbox.checked) {
  ...
  if (confirmResult) {
    ...
    console.log(newCommentData.type);  //확인하면 hide로 나옴
  } else {
    $checkbox.checked = false;
    newCommentData.type = 'show';
    console.log(newCommentData.type); // show로 나옴 
  }
} else {
  newCommentData.type = 'show';
}
...
function renderComment(data) {
 ...
 if (item.type === 'hide') {
  ...
  console.log(item.type);  //show로 나옴 
} 
    ...
    $commentContent.addEventListener('click', () => {
      if (item.type === 'hide') {
        $commentContent.textContent = item.content;
        item.type = 'show'; // 클릭하면 비밀글에서 보이는 글로 변경
